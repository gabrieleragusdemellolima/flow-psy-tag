...
import { CreditCard, Banknote, QrCode, Smartphone, CheckCircle2, Usb, Unplug, Wifi, AlertTriangle } from 'lucide-react';
import {
  connectACR122U,
  disconnectACR122U,
  pollForTag,
...
export default function LoadTag() {
  ...
  const handleConnectReader = useCallback(async () => {
    ...
    try {
      ...
      console.error('Erro ao conectar ACR122U:', error);
  ...
  const handleLoad = async () => {
    ...
    if (!tagCode || Number.isNaN(val) || val <= 0 || !user) return;
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {readerConnected ? (readerPolling ? '🟢 Lendo tags em tempo real' : 'Conectado') : 'Desconectado'}
                </p>
              </div>
            </div>
